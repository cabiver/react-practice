...
Router.post('/', async (req, res) => {
  ...
  const primer = new model({
    usuari: op.uss,
    password: op.contra
  })
  const user = await model.findOne({ usuari: primer.usuari })
  if (user) {
...
Router.post('/recomendacion', async (req, res) => {
  const op = req.body
  const users = await model.find({ usuari: { $regex: `${op.usuarios}` } }, { usuari: 1, _id: 0 }).limit(4)
  const arrayUsers = []
...
Router.post('/UltimosPost', async (req, res) => {
  ...
  for (let i = 0; i <= user.ultimasBusquedas.length - 1 && numeroPost > 0; i++) {
    const userAmigo = await model.findOne({ usuari: user.ultimasBusquedas[i] })
  ...
  for (let i = 0; numeroPost > 0; numeroPost--) {
    ...
    const userAmigo = await model.findOne({ _id: user.amigos[random] })
    let existeAmigo = false
    arrayAmigos.forEach(el => {
      if (el.nombre == userAmigo.usuari) {
        existeAmigo = true
...
Router.post('/:id', async (req, res) => {
  ...
  const user = await model.findOne({ usuari: objetoVerificacion.decodedToken.usuariname })
  const userAmigo = await model.findOne({ usuari: nombreAmigo })
  if (!userAmigo) {
